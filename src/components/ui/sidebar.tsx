...
function SidebarProvider({
  ...
  // Track whether the window is narrow enough that expanding the sidebar should
  // overlay content rather than push it. Above 1024px = inline; below = overlay.
  ...
  // Keep a ref so the collapse action always uses the latest setOpen without
  // making it a dependency — the effect must only fire on isNarrow transitions,
  // not every time setOpen is recreated (which would fight user-initiated opens).
  const setOpenRef = React.useRef(setOpen)
  React.useLayoutEffect(() => { setOpenRef.current = setOpen }, [setOpen])

  // Auto-collapse to icon rail when entering narrow mode.
  React.useEffect(() => {
    if (isNarrow) setOpenRef.current(false)
  }, [isNarrow])
  ...
  const contextValue = React.useMemo<SidebarContextProps>(
    ...
    [state, open, setOpen, isMobile, isNarrow, openMobile, setOpenMobile, toggleSidebar]
  )
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          {...props}
        >
          {/* Backdrop — shown when sidebar is expanded as an overlay in narrow mode */}
          {isNarrow && open && (
            <div
...
function Sidebar({
  ...
  const { state, isNarrow } = useSidebar()
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          // Narrow mode: always icon-width — sidebar panel overlays content when expanded.
          // Wide mode: standard logic — icon-width when collapsed, full-width when expanded.
